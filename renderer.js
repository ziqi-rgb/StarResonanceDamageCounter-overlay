document.addEventListener('DOMContentLoaded', async () => {
    ...
    const ALL_STATS_CONFIG = {
        realtime_dps: { label: '实时DPS', type: 'simple' },
        total_dps:    { label: '总秒伤', type: 'simple' },
        total_damage: { label: '总伤害', type: 'simple' },
        realtime_hps: { label: '实时HPS', type: 'simple' },
        total_hps:    { label: '总秒疗', type: 'simple' },
        total_healing:{ label: '总治疗', type: 'simple' },
        taken_damage: { label: '承受伤害', type: 'simple' },
        profession:   { label: '职业', type: 'simple' },
        fight_point:  { label: '总评分', type: 'simple' },
        peak_dps:     { label: '峰值DPS', type: 'simple' },
        hp_bar:       { label: '生命值', type: 'progress' }
    };
    const appState = {
        targetUid: null, lastData: null, lastDatalistUIDs: [],
        isMainWindow: false, nicknames: {}, currentDisplayUid: null,
        isEditingTitle: false, lastNotifiedHeight: 0,
        isLocked: false, favoriteUid: null, lastBroadcastedUid: null,
    };
    ...
    const elements = {
        ...
        skillDetailsBtn: document.getElementById('skill-details-btn'),
        editTitleBtn: document.getElementById('edit-title-btn'),
        closeBtn: document.getElementById('close-btn'),
        userControlPanel: document.getElementById('user-control-panel'), 
        dropdownToggleBtn: document.getElementById('dropdown-toggle-btn'),
        uidInput: document.getElementById('uid-input'),
        customDropdown: document.getElementById('custom-dropdown'),
        retargetUidBtn: document.getElementById('retarget-uid-btn'),
        trackUidBtn: document.getElementById('track-uid-btn'),
        statsContainer: document.getElementById('stats-container'),
        statusMessage: document.getElementById('status-message'),
        clearStatsBtn: document.getElementById('clear-stats-btn'),
        teamStatsToggleBtn: document.getElementById('team-stats-toggle-btn'),
    ...
    function initializeMainWindowControls() {
        elements.dropdownToggleBtn.addEventListener('click', (event) => {
            event.stopPropagation();
            if (appState.lastDatalistUIDs.length > 0) {
                elements.customDropdown.classList.toggle('hidden');
            }
        });
        elements.customDropdown.addEventListener('click', (event) => {
            const option = event.target.closest('.dropdown-option');
            if (option) {
                elements.uidInput.value = option.dataset.uid;
                elements.customDropdown.classList.add('hidden');
            }
        });
        elements.retargetUidBtn.addEventListener('click', () => {
            appState.targetUid = elements.uidInput.value.trim();
            updateUI(appState.lastData);
        });
        elements.trackUidBtn.addEventListener('click', () => {
            const uidToTrack = elements.uidInput.value.trim();
            if (uidToTrack) {
                window.electronAPI.createNewWindow(uidToTrack);
            }
        });
        elements.uidInput.addEventListener('keydown', (event) => {
            if (event.key === 'Enter') {
                elements.trackUidBtn.click();
            }
        });
        document.addEventListener('click', () => {
            if (!elements.customDropdown.classList.contains('hidden')) {
                elements.customDropdown.classList.add('hidden');
            }
        });
    }
    ...
    function loadAndApplyTheme() {
        const themeSettings = {
            opacity: localStorage.getItem('theme_opacity') || '0.85',
            bgColor: localStorage.getItem('theme_bgColor') || '#1e1e28',
            isBold: localStorage.getItem('theme_isBold') === 'true',
            isItalic: localStorage.getItem('theme_isItalic') === 'true',
        };
    ...
    function updateCustomDropdown(uids, userData) {
        ...
        if (sortedNewUIDs.join(',') !== appState.lastDatalistUIDs.join(',')) {
            ...
            uids.forEach(uid => {
                ...
                optionDiv.dataset.uid = uid;
                const name = userData[uid]?.name || appState.nicknames[uid] || userData[uid]?.profession || '未知';
                optionDiv.textContent = `${name} (${uid})`;
                optionDiv.title = optionDiv.textContent;
    ...
    function updateWindowTitle(uid, userData, customTitle = null) {
        if (customTitle) {
            elements.windowTitleText.textContent = customTitle;
            elements.editTitleBtn.classList.add('hidden');
            elements.skillDetailsBtn.classList.add('hidden');
            elements.favoriteBtn.classList.add('hidden');
            return;
        }
        if (!uid || !userData) {
            elements.windowTitleText.textContent = '伤害统计';
            elements.editTitleBtn.classList.add('hidden');
            elements.skillDetailsBtn.classList.add('hidden');
            elements.favoriteBtn.classList.add('hidden');
            return;
        ...
        elements.skillDetailsBtn.classList.remove('hidden');
        if (appState.isMainWindow) elements.favoriteBtn.classList.remove('hidden');
    }

    function toggleTitleEditMode() {
        const uid = appState.currentDisplayUid;
        if (!uid) return;
        if (appState.isEditingTitle) {
            const input = elements.titleContainer.querySelector('.title-input');
            if (!input) return;
            window.electronAPI.setNickname(uid, input.value.trim()).then(() => {
                ...
                elements.editTitleBtn.textContent = '✏️';
    ...
    function initializeAndRender(userData) {
        ...
        for (const statId in ALL_STATS_CONFIG) {
            ...
            if (config.type === 'simple') {
                ...
                barText.className = 'progress-bar-text';
                
                const hp = userData.hp || 0;
                const max_hp = userData.max_hp || 0;
                const percentage = (max_hp > 0) ? (hp / max_hp) * 100 : 0;
    ...
    function updateFavoriteButtonState() {
        ...
        if (appState.currentDisplayUid) {
            ...
            if (appState.currentDisplayUid.toString() === appState.favoriteUid) {
                elements.favoriteBtn.textContent = '★';
                elements.favoriteBtn.title = '取消收藏';
            } else {
                elements.favoriteBtn.textContent = '☆';
                elements.favoriteBtn.title = '收藏为默认角色';
            }
        } else {
            elements.favoriteBtn.classList.add('hidden');
        }
    ...
    function updateUI(data) {
        ...
        if (appState.isMainWindow && uids.length > 0) {
            updateCustomDropdown(uids, data.user);
        }
        let uidToShow = null;
        if (appState.isMainWindow && appState.targetUid) {
            uidToShow = data?.user?.[appState.targetUid] ? appState.targetUid : null;
        } else {
             const targetExists = data?.user?.[appState.targetUid];
             if (targetExists) uidToShow = appState.targetUid;
        }
        if (!uidToShow && appState.isMainWindow && uids.length > 0) {
            uidToShow = uids[0];
        }
        if (appState.currentDisplayUid !== uidToShow && appState.isEditingTitle) {
            toggleTitleEditMode();
        }
        ...
        if (uidToShow) {
            ...
            if (appState.isMainWindow && data) {
                title = '伤害统计 (等待中)'; message = '等待玩家数据...';
            } else if (!data) {
                title = '伤害统计 (未连接)'; message = '无法连接到后端服务...';
            } else {
                title = `伤害统计 (等待 ${appState.targetUid})`; message = `正在等待UID [${appState.targetUid}] 的数据...`;
            }
    ...
    window.electronAPI.onSettingsUpdated(() => {
        loadAndApplyTheme();
        if (appState.currentDisplayUid && appState.lastData) {
            initializeAndRender(appState.lastData.user[appState.currentDisplayUid]);
        }
    });
    elements.favoriteBtn.addEventListener('click', async () => {
        if (appState.currentDisplayUid) {
            const newFavorite = await window.electronAPI.toggleFavoriteUid(appState.currentDisplayUid);
            appState.favoriteUid = newFavorite ? newFavorite.toString() : null;
            updateFavoriteButtonState();
        }
    });
    elements.clearStatsBtn.addEventListener('click', async () => {
        if (confirm('您确定要清除所有统计数据吗？此操作不可撤销。')) {
            await window.electronAPI.clearStatsData();
        }
    });
    elements.skillDetailsBtn.addEventListener('click', () => {
        if (appState.currentDisplayUid) {
            window.electronAPI.toggleSkillDetailsWindow(appState.currentDisplayUid);
        }
    });
    ...
    if (uidFromUrl === 'main') {
        ...
        appState.targetUid = targetFromUrl || null;
        
        initializeMainWindowControls();
        elements.teamStatsToggleBtn.addEventListener('click', () => {
            window.electronAPI.toggleTeamStatsWindow();
        });
        ...
        elements.userControlPanel.style.display = 'none';
        elements.teamStatsToggleBtn.style.display = 'none';
        elements.clearStatsBtn.style.display = 'none';
